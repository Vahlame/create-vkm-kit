...
`npm run setup` ([`scripts/agent-install.mjs`](./scripts/agent-install.mjs)) **preflights** dependencies (`node`, `uv`, `git`, `python`/`pip`), **auto-detects** which agent CLIs are on PATH (`codex`, `claude`; falls back to Cursor's `mcp.json`), runs the `--full` install — hybrid + semantic + index + rules, plus for Claude Code the token-saver hooks, local telemetry/`vkm-doctor`, the `/vkm-discipline`, `/vkm-spec`, `/vkm-design` & `/vkm-research` skills and the memory-enforcement hooks; or the basic-memory stack if Python is absent — then **verifies** (vault scaffolded, index built, Python backend importable, `codex/claude mcp list` shows the servers) and prints a status table. Pass options after `--`: `npm run setup -- --vault "<path>" --ide codex,claude --lang en`. Use `npm run setup:dry` to preview with zero writes.
...
#   node packages/create-vkm-kit/src/index.js --full --vault <vault> --repo-root .
# …or spell it out:
