...
Una herramienta [MCP](#mcp) (expuesta por [`obsidian-memory-hybrid`](#obsidian-memory-hybrid)) que arma un **paquete de contexto presupuestado en una sola llamada** en vez de encadenar búsquedas: la sección relevante de la nota del proyecto, los pasajes que mejor coinciden y pistas de stack — todo passage-first y con tope de caracteres. Mediana de **−68 % de tokens de wire** frente a encadenar búsquedas, con gate en CI. También alimenta a [`vkm-spec`](#vkm-spec). Ver ADR-0045.
...
Estructura consultable por el agente, escrita como Markdown plano (compatible con Basic Memory): **relaciones tipadas** (`- implements [[nota]]`) y **observaciones categorizadas** (`- [decision] hecho #tag`). Se consulta con las herramientas [MCP](#mcp) `vault_relations`, `vault_observations` y `vault_kg_suggest` (sugerencias de solo lectura). Ver ADR-0023.
...
### Memory report (Reporte de memoria)

Un resumen de solo lectura para el mantenimiento periódico del vault (`vault_memory_report`): índices automáticos (observaciones por categoría, notas hub, `#tags` principales), señales de higiene (notas sobredimensionadas / obsoletas / huérfanas, links rotos, hinchazón de `SESSION_LOG`) con pasos concretos, y pares casi duplicados opt-in. Señala qué condensar; nunca reescribe una nota. Ver ADR-0024.
...
### Token-saver (Ahorrador de tokens)

Dos hooks `PostToolUse` (activos por defecto en Claude Code) que compactan la salida ruidosa de las herramientas **antes** de que entre al contexto del modelo — limpieza de ANSI/progreso/líneas duplicadas preservando duro las líneas de error/warn/fail, más compactación de espacios del JSON de MCP — junto con reglas `permissions.deny` para artefactos devoradores de tokens y el output style `vkm-terse`. Con gate en CI: **≥30 % de compactación con cero pérdida de diagnóstico**. Interruptor de emergencia: `VKM_TOKEN_SAVER=0`. Ver ADR-0043.
...
El nombre de esta suite desde la 4.0: la memoria persistente en Markdown+git que describe este glosario **más** las piezas de eficiencia — el [token-saver](#token-saver-ahorrador-de-tokens), [`vkm-doctor`](#vkm-doctor), [`vkm-spec`](#vkm-spec), las [skills](#skills-habilidades) y la capa opcional [`obscura-web`](#obscura-web) — todo instalado por `npx @vkmikc/create-vkm-kit`. Los nombres de compatibilidad están congelados a propósito: la clave MCP `obsidian-memory-hybrid`, `obsidian-memoryd`, [`BASIC_MEMORY_HOME`](#basic_memory_home) y las variables `OBSIDIAN_MEMORY_*` siguen funcionando. Ver la [guía de migración a 4.0](./migracion-4.0.md).

### `vkm-spec`

El constructor de specs de la suite (`packages/vkm-spec`): convierte una idea en un **XML `<orchestration_package>` anclado al vault** que pegas en tu agente. GUI local en `127.0.0.1:4923` (`vkm-spec --gui`) o CLI (`vkm-spec "<idea>"`); un Ollama local opcional (`phi4-mini`) redacta con structured outputs, y un **fallback determinista** produce una spec válida siempre que Ollama no esté — un invariante testeado en CI, no una promesa. Ver ADR-0046 / ADR-0047 / ADR-0048.
