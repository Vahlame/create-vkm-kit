# Changelog — 3.15.0 and earlier

The releases before 4.0.0, moved out of the root [`CHANGELOG.md`](../../CHANGELOG.md) in
5.0.0. Nothing was edited: this is the same text, and the same
[Keep a Changelog](https://keepachangelog.com/en/1.1.0/) format.

It was split because the root file had reached 248 KB in one document, of which 92 KB
covered releases of an npm name that has been deprecated and frozen since 4.0.0
(ADR-0050). Splitting is safe for the release machinery, which reads only the newest
`## [X.Y.Z]` heading (`scripts/version.mjs`) or the one section matching the tag being
published (`release.yml`) — both of which stay in the root file, always.
...
- **v3 kit layout (same branch: `main`):** the repository **no longer ships** Windows integration files under **`scripts/windows/`** (`.ps1`, `.vbs`) or convenience scripts under **`tools/*.ps1`**. The **advanced** setup (MCP stdio/HTTP, vault git, FTS hybrid) is unchanged in intent and is documented without those artifacts. Migration: [`docs/legacy/v2-to-v3-script-free-kit.md`](../legacy/v2-to-v3-script-free-kit.md). Maintainers still use **`scripts/sync-agents.ts`** (TypeScript) and **`.github/scripts/extract-and-lint.ps1`** (CI against the archived v1 prompt).
...
- **Docs onboarding:** `docs/troubleshooting.md` alineado a **v2** (MCP `basic-memory`, recuperación sin flujo v1); v1 solo como referencia en `docs/legacy/`. `README.md` / `README.en.md` — paso opcional a [`memory-repo-sin-automatismos-locales`](../es/sincronizacion.md).
...
- **Capítulo v2 → v3:** guía pública **stdio + `obsidian-memoryd` / git manual** por defecto; HTTP y tareas Windows como opciones **definidas por quien instala**. [`docs/legacy/v2-to-v3-script-free-kit.md`](../legacy/v2-to-v3-script-free-kit.md).
