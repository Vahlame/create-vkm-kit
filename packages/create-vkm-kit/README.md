...
| `--memory-enforcement` / `--no-memory-enforcement` | (Claude Code) Force on / skip the two deterministic-enforcement hooks (ADR-0030): a PreToolUse guard that **denies** writes into the native auto-memory + a Stop-hook close-ritual nudge.                                                                                                                                                                                                                            |
| `--effort-gate` / `--no-effort-gate`               | (Claude Code) Force on / skip the effort-gate hook (ADR-0031): denies a session's 2nd+ substantive edit until the model proposes an effort level and the user actually replies.                                                                                                                                                                                                                                      |
| `--token-saver` / `--no-token-saver`               | (Claude Code) Force on / remove the token-saver (ADR-0043): PostToolUse compaction hooks (Bash + `mcp__.*`) + the `vkm-terse` output style. The `permissions.deny` noise rules were retired (ADR-0043 amendment) and any run now removes leftovers from old installs. Runtime kill switch: `VKM_TOKEN_SAVER=0`.                                                                                                      |
...
| `--skills` / `--no-skills`                         | (Claude Code) Force on / remove the five skills — `/vkm-discipline`, `/vkm-spec`, `/vkm-design`, `/vkm-research`, `/vkm-verify` (ADR-0049/0053/0056/0080). Hash-tracked; uninstall never deletes one you edited.                                                                                                                                                                                                     |
| `--agents` / `--no-agents`                         | (Claude Code) Force on / remove the `vkm-implementer` subagent template.                                                                                                                                                                                                                                                                                                                                             |
...
| `--check-update`                                   | **Read-only self-update check (ADR-0061).** Prints installed vs. npm-latest `create-vkm-kit` and, for every managed file under `~/.claude/skills/` and `~/.claude/agents/`, whether it is up to date, missing, kit-changed, or locally modified. Writes nothing; a registry/offline failure prints an honest "skipped" line and it **always exits 0**.                                                               |
...
| `--uninstall`                                      | One-shot teardown of everything this kit's Claude Code integration installed (managed hook entries + the `autoMemoryEnabled` override + hook script files — each only after a marker check confirms the kit wrote it). Does **not** touch MCP registrations, the vault, or rules blocks. Combine with `--dry-run` to preview.                                                                                        |
