...
| `--token-saver` / `--no-token-saver`               | (Claude Code) Force on / remove the token-saver (ADR-0043): PostToolUse compaction hooks (Bash + `mcp__.*`) + `permissions.deny` noise rules + the `vkm-terse` output style. Runtime kill switch: `VKM_TOKEN_SAVER=0`.                                                                                                                                             |
