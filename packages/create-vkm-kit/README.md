...
Beyond memory, the same command installs the rest of the **vkm-kit efficiency suite** for
Claude Code: **token-saver** compaction hooks + the `vkm-terse` output style (ADR-0043), a
**local telemetry sink + usage doctor** (`vkm-doctor`, ADR-0044), the **`/vkm-discipline`,
`/vkm-spec`, `/vkm-design` and `/vkm-research` skills** + the `vkm-implementer` subagent (ADR-0049/0053/0056), and
the opt-in **obscura web layer** (`--obscura`, ADR-0051).

Part of the [vkm-kit](https://github.com/Vahlame/create-vkm-kit)
suite. Full docs (English + Spanish), architecture and ADRs live there.
...
| `--minimal`                                        | Opposite of the default: plain `basic-memory` only — no hybrid, semantic, vec, index, rules, hooks, skills, or telemetry. Re-add a piece with its individual flag.                                                                                                                                                                                                                                                   |
...
| `--repo-root <path>`                               | Root of the kit clone (hybrid bridge + Python source).                                                                                                                                                                                                                                                                                                                                                               |
...
| `--pin-failures` / `--no-pin-failures`             | With `--with-hybrid`: resurface recorded `KNOWN_FAILURES` lessons on matching tasks (sets `OBSIDIAN_MEMORY_PIN_FAILURES=1`; ADR-0038). **On by default.**                                                                                                                                                                                                                                                            |
| `--usage-boost` / `--no-usage-boost`               | With `--with-hybrid`: boost notes the agent demonstrably used, from the local recall telemetry (sets `OBSIDIAN_MEMORY_USAGE_BOOST=1`; ADR-0038). **On by default.**                                                                                                                                                                                                                                                  |
...
| `--codex-hooks` / `--no-codex-hooks`               | (Codex) Force on / remove `~/.codex/hooks.json`: SessionStart vault context, PreToolUse memory/effort guards, and PostToolUse token-saver. User hook entries are preserved.                                                                                                                                                                                                                                          |
| `--vault-context-hook` / `--no-vault-context-hook` | (Codex) Force on / remove just the SessionStart vault-context injector.                                                                                                                                                                                                                                                                                                                                              |
| `--memory-enforcement` / `--no-memory-enforcement` | (Claude Code + Codex) Force on / skip native-memory guards. Claude additionally has its Stop-hook close-ritual nudge; Codex protects generated `~/.codex/memories/` paths.                                                                                                                                                                                                                                           |
| `--effort-gate` / `--no-effort-gate`               | (Claude Code + Codex) Force on / skip the effort gate (ADR-0031). Codex applies it to `apply_patch` through `PreToolUse`.                                                                                                                                                                                                                                                                                            |
| `--token-saver` / `--no-token-saver`               | (Claude Code + Codex) Force on / remove the token-saver (ADR-0043). Claude also receives `vkm-terse`; Codex returns compacted PostToolUse feedback because `updatedToolOutput` is not currently supported. Runtime kill switch: `VKM_TOKEN_SAVER=0`.                                                                                                                                                                 |
| `--terse-style` / `--no-terse-style`               | (Claude Code) Just the `vkm-terse` output style on / off, independent of the compaction hooks.                                                                                                                                                                                                                                                                                                                       |
| `--telemetry` / `--no-telemetry`                   | (Claude Code) Force on / remove the local telemetry sink + managed OTEL env block (`vkm-doctor`, ADR-0044). All local: `127.0.0.1:4319` → `~/.vkm/telemetry/`, nothing leaves the machine.                                                                                                                                                                                                                           |
| `--skills` / `--no-skills`                         | (Claude Code + Codex) Force on / remove the five skills — `/vkm-discipline`, `/vkm-spec`, `/vkm-design`, `/vkm-research`, `/vkm-verify` (ADR-0049/0053/0056/0080). Codex receives byte-identical directories under `~/.agents/skills/`. Hash-tracked; uninstall never deletes one you edited.                                                                                                                        |
| `--agents` / `--no-agents`                         | (Claude Code + Codex) Force on / remove `vkm-implementer`: Markdown under `~/.claude/agents/` and required-key TOML under `~/.codex/agents/`.                                                                                                                                                                                                                                                                        |
| `--ollama` / `--no-ollama`                         | Install Ollama + pull `phi4-mini` for `vkm-spec` drafts (best-effort, multi-GB download — only under explicit `--ollama` or `--full`; `--no-ollama` wins).                                                                                                                                                                                                                                                           |
| `--help`                                           | Show usage.                                                                                                                                                                                                                                                                                                                                                                                                          |
| `--obscura`, `--no-obscura`                        | Install + wire **obscura-web** (stealth `obscura_fetch` + robust `obscura_search`); **on under `--full`**. Needs a kit clone; downloads + SHA-256-verifies the pinned obscura binary into `~/.vkm/obscura/`. See [ADR-0051](https://github.com/Vahlame/create-vkm-kit/blob/main/docs/adr/0051-obscura-web-stealth-browser.md).                                                                                       |
...
| `--downloads` / `--no-downloads`                   | Wire **vkm-downloads** (guarded `download_resolve` → confirm → download into `~/Downloads/vkm-kit/`; background jobs, resume, fastest-mirror — ADR-0058/0059). **Default off and deliberately outside `--full`** (it writes to disk). Needs a kit clone.                                                                                                                                                             |
| `--download-dir <dir>`                             | Override `VKM_DOWNLOAD_DIR` (default `~/Downloads/vkm-kit`).                                                                                                                                                                                                                                                                                                                                                         |
| `--check-update`                                   | **Read-only self-update check (ADR-0061).** Prints installed vs. npm-latest `create-vkm-kit` and checks managed Claude and Codex skill, agent and hook assets for missing, kit-changed or local modifications. Writes nothing; a registry/offline failure prints an honest "skipped" line and it **always exits 0**.                                                                                                 |
| `--update`                                         | **Apply that plan.** Installs the managed files that are missing or that the kit changed since your install; a file **you** edited is left untouched and listed by name. Add `--force` to reset those to the shipped versions (**discards your local edits**), or `--dry-run` to preview with zero writes. Covers the hash-guarded asset layer only — rules blocks and MCP registrations still need a normal re-run. |
| `--uninstall`                                      | One-shot teardown of managed Claude and Codex assets. It removes only vkm-kit Codex entries from `hooks.json`, removes hash-tracked Codex assets only when unmodified, and never touches user hooks. It does **not** touch MCP registrations, the vault, or rules blocks. Combine with `--dry-run` to preview.                                                                                                       |
...
Read the rules block before installing — it's printed in [`docs/en/install.md`](https://github.com/Vahlame/create-vkm-kit/blob/main/docs/en/install.md) (Step 4) and is what `--rules` writes (idempotently) into your agent configs.
...
Free use with mandatory visible attribution (MIT-based) — © Vahlame. See [`LICENSE.md`](./LICENSE.md) and the [repository](https://github.com/Vahlame/create-vkm-kit) for details.
