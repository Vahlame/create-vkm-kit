...
// These eleven tests spent a release skipped, while robots.mjs was a no-op stub and five
// places — ADR-0062 §3/§6, two MCP tool descriptions the model reads, the package README and
// a documented `OBSCURA_ROBOTS_CACHE_TTL_MS` env var — went on promising compliance. They are
// the spec for what `checkRobots` must do, and they are un-skipped because the implementation
// they describe is back. A compliance control gets restored, not documented away; if it is
// ever disabled again, every claim above has to come out in the same change.

// ── parseRobotsTxt / isPathAllowed: pure, no network ────────────────────────────────────

test("parseRobotsTxt groups Disallow/Allow under the User-agent that precedes them", () => {
...
test("parseRobotsTxt ignores comments, blank lines, and unrelated directives", () => {
  const records = parseRobotsTxt(
    [
      "# a comment",
      "User-agent: *",
...
test("findStarRecord picks the first * group; absent one, an empty (permissive) ruleset", () => {
  const withStar = parseRobotsTxt("User-agent: Bingbot\nDisallow: /\nUser-agent: *\nDisallow: /x");
...
test("isPathAllowed: a plain Disallow blocks its prefix", () => {
...
test("isPathAllowed: a longer, more specific Allow overrides a shorter Disallow", () => {
  const r = {
    agents: ["*"],
    rules: [
      { type: "disallow", path: "/admin" },
...
test("isPathAllowed: Disallow: / blocks everything absent a more specific Allow", () => {
...
test("checkRobots: blocks a disallowed path, allows an unlisted one", async () => {
  clearRobotsCache();
...
test("checkRobots: a 404 robots.txt means no restriction, and IS a checked result", async () => {
  clearRobotsCache();
...
test("checkRobots: caches per origin — a second URL on the same host doesn't re-fetch", async () => {
  clearRobotsCache();
  let calls = 0;
  const fetchImpl = async () => {
    calls++;
...
test("checkRobots: different origins are cached separately", async () => {
  clearRobotsCache();
  let calls = 0;
  const fetchImpl = async () => {
    calls++;
...
test("checkRobots: an expired cache entry re-fetches", async () => {
  clearRobotsCache();
  let calls = 0;
  const fetchImpl = async () => {
    calls++;
