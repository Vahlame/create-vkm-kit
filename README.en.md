...
<p align="center">
  ...
  <a href="./CHANGELOG.md"><img src="https://img.shields.io/badge/release-v5.5.2-orange.svg" alt="Release"></a>
...
an effort advisor that calibrates cost without ever interrupting: it persists the effort level the
work calls for so the next session starts there, and notifies you once outside the model's context (ADR-0081). Just the basics? Use `--ide codex,claude`. Full guide:
...
| [`packages/vkm-memory-pg/`](packages/vkm-memory-pg/)             | Node     | Postgres projection of the index **(opt-out `--no-postgres`; embedded PGlite, or your own server via `--pg-dsn`)**: multi-hop typed graph in one query, SQL analytics over the whole vault, a temporal activity log and live SSE events. Derived and disposable — the vault still governs (ADR-0084).                                                         |
| [`cmd/obsidian-memoryd/`](cmd/obsidian-memoryd/)                 | Go       | Optional daemon: watches the vault and syncs git.                                                                                                                                                                                                                                                                                                             |
| [`cmd/vkm-console/`](cmd/vkm-console/)                           | Go       | Whole-kit real-time console **(opt-in `--console`)**: daemon, memory, tokens, Postgres activity and research on `127.0.0.1:4930`. Single binary, strictly read-only, and it **opens no window** unless you pass `--open` (ADR-0085).                                                                                                                          |
...
>
> 🧭 **Skills the kit installs** (besides the packages): **`/vkm-discipline`** — cross-domain execution discipline (infer the real intent, deliver more than the literal ask, with run evidence) that lifts **any model**, Haiku to Opus — **`/vkm-spec`** (idea → a spec anchored to the vault) — **`/vkm-design`** (professional, anti-generic design for any UI/medium: direction before pixels, computed checks, real libraries verified online, a visual loop) — **`/vkm-research`** (consolidates a `RESEARCH/<topic>` bank into one `summary.md` with wikilinks and supersession) — **`/vkm-verify`** (proves a green check actually ran, covered your change and can fail: negative control via `prove-it.mjs`) — **`/vkm-intake`** (read the task right before executing: objective/deliverable/non-goals in 3 lines, one closed question max on ambiguity, an inventory of what attached images actually show, minimal context) — **`/vkm-seo`** (brutal, measurable SEO for websites: semantic coverage of synonyms/variants/locations, technical + schema + AI-search visibility, with a static before/after audit) — and **`/vkm-ui-judge`** (**measured** visual judgment for any GUI: web via the Playwright audit at 3 viewports × light/dark with computed WCAG contrast; Flutter via `flutter_test`'s accessibility gates; Qt/.NET/Python/Java via a real-screenshot loop — fixes with before/after evidence instead of "thinking at the screen"). Which one fits a situation: [skills guide](docs/en/skills-guide.md). Detail: [ADR-0049](docs/adr/0049-discipline-doctrine-three-channels.md), [ADR-0053](docs/adr/0053-vkm-design-skill.md) and [ADR-0056](docs/adr/0056-research-knowledge-bank.md).
...
the 4 skills in that bench route with **100% hit-rate and 0% false-positives** (104 ES+EN cases; `/vkm-verify`, `/vkm-intake` and `/vkm-ui-judge` postdate it and are not measured yet); the
