...
| [`AGENTS/vkm-implementer.md`](./AGENTS/vkm-implementer.md) | A per-agent memory note (ADR-0086): verdicts and lessons for a delegated executor, recalled with `scope: "AGENTS/vkm-implementer"`.                                                                                                                                                                          |
...
| **FTS index (optional)**                                   | If you use `obsidian-memory-hybrid`, ignore `.obsidian-memory-rag/` via `.gitignore` (already listed). v3 is **passage-first**: `vault_hybrid_search` returns the relevant section, not the whole note. As the log grows, `obsidian-memory-rag rotate-log` archives old entries to `SESSION_LOG/archive.md`. |
