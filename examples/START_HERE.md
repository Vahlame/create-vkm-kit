...
- **Salud del vault:** `vault_audit` valida frontmatter, wikilinks rotos y notas huérfanas; fusiona el híbrido con el inicializador `create-vkm-kit --ide cursor,claude --with-hybrid --build-index --semantic --repo-root <clon-del-kit>` (necesita `pip install -e packages/obsidian-memory-rag` previo; ver `docs/es/instalacion.md`).
